...
export function ProjectCard({ project, featured = false }: ProjectCardProps) {
  return (
    <article
      className={`group flex h-full flex-col rounded-lg border border-white/10 bg-white/[0.035] p-5 transition-colors hover:border-emerald-300/40 hover:bg-white/[0.055] sm:p-6 ${
      ...
      <div className="mt-6 flex flex-wrap gap-3 pt-2">
        {project.links.map((link) => (
          <a
            ...
            className="rounded-md border border-white/10 px-3.5 py-2 text-sm font-medium text-zinc-200 transition-colors hover:border-emerald-300/50 hover:bg-emerald-300/10 hover:text-white"
          >
            {link.label}
          </a>
        ))}
